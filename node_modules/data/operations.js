(function () { 
  ...
  function filter(series, pred) {
    ...
    const key = newLen === len ? series.key 
          : new data.Key(newLabels.slice(0, newLen), series.key.name)
    return new data.Series(key, newValues.slice(0, newLen), series.name)
  }

  function map(series, fn, name) {
    const len = series.length
    const labels = series.key.labels
    const values = series.values
    const res = new series.values.constructor(len)
    for (let i = 0; i < len; ++i) res[i] = fn(values[i], labels[i])
    return new data.Series(series.key, res, name)
  }
